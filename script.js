...
window.openModal = function(index) {
    ...
    // ЛОГИКА ДЛЯ SOLD ТОВАРОВ:
    // Ищем кнопку "В корзину" внутри модалки
    const buyBtn = modal.querySelector('button[onclick="addToCart()"]');
    if (currentProduct.sold) {
        buyBtn.style.display = 'none'; // Скрываем кнопку, если продано
    } else {
        buyBtn.style.display = 'block'; // Показываем, если в наличии
    }
...
window.addToCart = function() {
    // Добавлена проверка на sold, чтобы нельзя было добавить через консоль
